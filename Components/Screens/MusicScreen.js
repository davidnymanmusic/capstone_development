...
import { Text, View, StyleSheet, TouchableHighlight, TouchableOpacity, Button, Image, Dimensions, Slider, ScrollView, Modal } from 'react-native';
...
export default class MusicScreen extends React.Component {
  ...
  render() {
    return (
      <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: MAIN_COLOR }}>
		...
		<TouchableOpacity onPress={this.timeReducer}><Ionicons name='minus-circle' size={35} color='#fff'/></TouchableOpacity>
		<Text style={styles.textTimer}>Timer</Text>
				<TouchableOpacity onPress={this.timePicker}><Ionicons name='plus-circle' size={35} color='#fff'/></TouchableOpacity>
					</View>
				...
				{this.state.pressed && this.state.timer !== 0 ?
						<TimerCountdown initialSecondsRemaining={this.state.count}
						...
						allowFontScaling={true} style={{ fontSize: 20 }}></TimerCountdown> :
						 <Text style={{fontSize: 20, marginTop: 20}}>{this.state.timer}</Text>
				}
				</View>
				...
				<View style={{marginTop: 22}}>
					<Modal
						...
						<View style={styles.container}>
							...
							<ScrollView style={{ flex: 1, width: width / 1.1, backgroundColor: MAIN_COLOR, borderColor: 'rgba(255, 255, 255, 0.5)', borderWidth: 2, margin: 2}}>
